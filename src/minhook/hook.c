...
static VOID ProcessThreadIPs(HANDLE hThread, UINT pos, UINT action)
{
    ...
    if (pos == ALL_HOOKS_POS) { pos = 0; count = g_hooks.size; }
    else { count = pos + 1; }
    for (; pos < count; ++pos)
    {
        PHOOK_ENTRY pHook = &g_hooks.pItems[pos];
        BOOL enable; DWORD_PTR ip;
        switch (action) {
        ...
        if (enable) ip = FindNewIP(pHook, *pIP);
        else ip = FindOldIP(pHook, *pIP);
...
static BOOL EnumerateThreads(PFROZEN_THREADS pThreads)
{
    ...
    if (hSnapshot != INVALID_HANDLE_VALUE)
    {
        ...
        if (Thread32First(hSnapshot, &te))
        {
            ...
            if (!succeeded && pThreads->pItems != NULL) { HeapFree(g_hHeap, 0, pThreads->pItems); pThreads->pItems = NULL; }
        }
...
static MH_STATUS Freeze(PFROZEN_THREADS pThreads, UINT pos, UINT action)
{
    ...
    if (!EnumerateThreads(pThreads)) status = MH_ERROR_MEMORY_ALLOC;
    else if (pThreads->pItems != NULL)
    {
        ...
        for (i = 0; i < pThreads->size; ++i)
        {
            ...
            if (hThread != NULL)
            {
                ...
                if (result != 0xFFFFFFFF) { suspended = TRUE; ProcessThreadIPs(hThread, pos, action); }
                CloseHandle(hThread);
            }
            if (!suspended) pThreads->pItems[i] = 0;
        }
    }
    return status;
}
    ...
    if (pThreads->pItems != NULL)
    {
        ...
        for (i = 0; i < pThreads->size; ++i)
        {
            ...
            if (threadId != 0)
            {
                ...
                if (hThread != NULL) { ResumeThread(hThread); CloseHandle(hThread); }
            }
        }
...
static VOID EnterSpinLock(VOID)
{
    ...
    {
        if (spinCount < 32) Sleep(0); else Sleep(1);
        spinCount++;
    }
}
...
MH_STATUS WINAPI MH_Initialize(VOID)
{
    ...
    if (g_hHeap == NULL)
    {
        ...
        if (g_hHeap != NULL) InitializeBuffer();
...
static MH_STATUS QueueHook(LPVOID pTarget, BOOL queueEnable)
{
    ...
    if (g_hHeap != NULL)
    {
        if (pTarget == MH_ALL_HOOKS) { UINT i; for (i = 0; i < g_hooks.size; ++i) g_hooks.pItems[i].queueEnable = queueEnable; }
        else { UINT pos = FindHookEntry(pTarget); if (pos != INVALID_HOOK_POS) g_hooks.pItems[pos].queueEnable = queueEnable; else status = MH_ERROR_NOT_CREATED; }
    }
    else status = MH_ERROR_NOT_INITIALIZED;
    LeaveSpinLock();
    return status;
...
MH_STATUS WINAPI MH_ApplyQueued(VOID)
{
    ...
    {
        for (i = 0; i < g_hooks.size; ++i)
            if (g_hooks.pItems[i].isEnabled != g_hooks.pItems[i].queueEnable) { first = i; break; }
        if (first != INVALID_HOOK_POS)
        {
            FROZEN_THREADS threads;
            ...
            if (status == MH_OK)
            {
                for (i = first; i < g_hooks.size; ++i)
                {
                    ...
                    if (pHook->isEnabled != pHook->queueEnable) { status = EnableHookLL(i, pHook->queueEnable); if (status != MH_OK) break; }
                }
                Unfreeze(&threads);
            }
        }
...
#define MH_ST2STR(x) case x: return #x;
    switch (status) {
        MH_ST2STR(MH_UNKNOWN) MH_ST2STR(MH_OK)
        MH_ST2STR(MH_ERROR_ALREADY_INITIALIZED) MH_ST2STR(MH_ERROR_NOT_INITIALIZED)
        MH_ST2STR(MH_ERROR_ALREADY_CREATED) MH_ST2STR(MH_ERROR_NOT_CREATED)
        MH_ST2STR(MH_ERROR_ENABLED) MH_ST2STR(MH_ERROR_DISABLED)
        MH_ST2STR(MH_ERROR_NOT_EXECUTABLE) MH_ST2STR(MH_ERROR_UNSUPPORTED_FUNCTION)
        MH_ST2STR(MH_ERROR_MEMORY_ALLOC) MH_ST2STR(MH_ERROR_MEMORY_PROTECT)
        MH_ST2STR(MH_ERROR_MODULE_NOT_FOUND) MH_ST2STR(MH_ERROR_FUNCTION_NOT_FOUND)
    }
#undef MH_ST2STR
    return "(unknown)";
}
